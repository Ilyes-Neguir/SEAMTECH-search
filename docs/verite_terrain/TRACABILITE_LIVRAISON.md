...
| CI 8 jobs verte sur la branche | `gh run view 35763512958` (push) et `gh run view 35763634670` (pull_request) | 8/8 `success` (backend 3.11/3.12/3.13, frontend, docker, integration, e2e, sauvegarde) ; étape postgres : passed=134 skipped=0 ; notice perf : `assistant_jeu_8_corpus_mixte : p50 = 1.7 ms, p95 = 2.0 ms, max = 4.3 ms (n = 80)` | 5a1510c (tête PR #24) | ✅ établi (deux runs verts, PR NON fusionnée) |

## Lot J — facette dimension, URL partageable, journal exploité (22/09/2026, `arena/01a0ca57-seamtech-search`) — itération 3

| Affirmation | Commande | Sortie brute | Commit | Statut |
|---|---|---|---|---|
| Facette DIMENSION : 7 cotes explicites avec unités métier, filtre par plage, intervalles depuis données réelles | `pytest -m postgres tests/test_recherche_dimension_tri.py` (CI) | 7 cotes (slu_m m, sle_m m, sf_m m, shw_m m, spa_m2 m², tetiere_cm cm, poids_kg kg), intervalles calculés, effectifs — run 35771238024 backend passed=140 | 45cbe22, babdeb5, 7676c4b | ✅ établi (CI backend 140 passed) |
| Filtre dimension compte sans son propre filtre (règle des facettes) | `test_dimension_facettes_et_intervalles` | effectif slu_m avec filtre = sans filtre, preuve garde-fou rouge→vert §2.9 rapport | 225516c | ✅ établi (logique + CI) |
| API étendue cote/min/max + alias cote_min/cote_max + tri 18 valeurs + page→offset | `pytest -m postgres tests/test_recherche_dimension_tri.py::test_api_dimension_params` + `test_tri_et_pagination` | codes filtrés {0701-GV-001, 0801-GEN-001}, page=2, tri code_asc — CI passed=140 | 45cbe22 | ✅ établi (CI) |
| Tri appliqué sur fusion AVANT pagination | `test_tri_et_pagination` | page1 max <= page2 min pour tri slu_m_asc — CI | 45cbe22 | ✅ établi |
| URL partageable : q+filtres+tri+page dans URL, restaurés, copier lien, F5, nouvel onglet | `pnpm exec playwright test e2e/recherche-url.spec.ts` | Fix `babdeb5` : replaceState AVANT fetch pour mode SQLite 503, sinon tri/page ne met pas à jour URL. 3 tests présents, build vert, tsc vert. Run 35771238024 e2e X avant fix, runs 35772537132/43594 après fix non consultables (token expiré) | babdeb5, 7676c4b | ⚠️ démontré localement, CI à re-prouver après reconnexion |
| Journal exploité : top requêtes + sans résultat agrégés depuis table réelle, période paramétrable | `pytest -m postgres tests/test_journal_recherche.py::test_journal_rapport_exactitude_sur_jeu_injecte` | jeu injecté 6 lignes → total 6, sans 3, top exact, période 1j — CI backend 140 passed inclut journal | 45cbe22 | ✅ établi |
| Route /recherche/journal + CLI recherche-log | `python -m seamtech_search.journal_recherche --help` + `GET /recherche/journal` | help OK, route retourne total_recherches, top, sans — 21 routes dont /api/recherche/journal | 225516c | ✅ établi (help mesuré) |
| Pas de régression : 502 passed/2 skipped hors postgres + ruff + audit 12/12 + tsc + build | `pytest -k "not postgres and not s3 and not perf"` + `ruff check` + `audit_projet.py --rapide` + `pnpm build` | 502 passed, 2 skipped, 182 deselected (mesuré §2.13), All checks passed, 12/12, build OK | 7676c4b | ✅ établi (local) |
| CI 7/8 verte après fix sauvegarde | `gh run view 35771238024` | docker ✓ 1m11s, backend 3.11/3.12/3.13 ✓ passed=140, integration ✓ 3m19s, frontend ✓ 35s, sauvegarde ✓ 1m10s, e2e X 3m25s — 7/8 verts | 45cbe22, babdeb5 | ✅ établi (preuve brute §2.11) |
| CI 8/8 verte Lot J | `gh run view 35772537132` + `35772543594` | queued → in_progress → 401 Bad credentials (token expiré) — logs et artefact Playwright inaccessibles | 7676c4b | ❌ NON PROUVÉ (token GitHub expiré, à rejouer après reconnexion Arena) |
| Sauvegarde version attendue 014 | `tests/test_sauvegarde_restauration.py` | version attendue mise à jour 014 après migration dimension, CI sauvegarde verte | 45cbe22 | ✅ établi |
