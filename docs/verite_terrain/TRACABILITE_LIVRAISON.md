...
## Lot J — facette dimension, URL partageable, journal exploité (22/09/2026, `arena/01a0ca57-seamtech-search`) — FINAL 8/8 verte

| Affirmation | Commande | Sortie brute | Commit | Statut |
|---|---|---|---|---|
| Facette DIMENSION : 7 cotes explicites avec unités métier, filtre par plage, intervalles depuis données réelles | `pytest -m postgres tests/test_recherche_dimension_tri.py` (CI) | 7 cotes (slu_m m, sle_m m, sf_m m, shw_m m, spa_m2 m², tetiere_cm cm, poids_kg kg), intervalles calculés, effectifs — run 35776638128 backend passed=140 | 45cbe22, babdeb5, 7676c4b, 3bd71d7 | ✅ établi (CI backend 140 passed) |
...
| URL partageable : q+filtres+tri+page dans URL, restaurés, copier lien, F5, nouvel onglet | `pnpm exec playwright test e2e/recherche-url.spec.ts` | Fix `babdeb5` : replaceState AVANT fetch pour mode SQLite 503 ; Fix `3bd71d7` : nouvel onglet partage cookies → /login redirige, ne pas exiger Password si invisible. 3 tests e2e verts en CI 1m58s | babdeb5, 3bd71d7 | ✅ établi (CI e2e vert) |
...
| Pas de régression : 502 passed/2 skipped hors postgres + ruff + audit 12/12 + tsc + build | `pytest -k "not postgres and not s3 and not perf"` + `ruff check` + `audit_projet.py --rapide` + `pnpm build` | 502 passed, 2 skipped, 182 deselected (mesuré §2.13), All checks passed, 12/12, build OK, CI 140 passed | 7676c4b, 3bd71d7 | ✅ établi (local + CI) |
