...
**Correctifs de couverture (audit indépendant du 23/09 sur `f5e1016`)** : (C.1) l'indicateur `doublons_detectes` n'avait AUCUN test et `tableau_de_bord()` / `GET /qualite/tableau-de-bord` non plus — trois tests ajoutés (`test_tableau_de_bord_expose_tous_les_indicateurs_declares` compare l'ENSEMBLE EXACT des clés à `CLES_TABLEAU_DE_BORD` — 9 après L.2 —, tests de valeurs avec et sans doublon, et test de la route) et la règle de maintenance est écrite dans `docs/API.md` : tout nouvel indicateur arrive avec sa clé dans le test de forme, son test de valeurs et sa ligne de doc ; (C.2) aucune spec Playwright ne regardait le bandeau de doublon — `frontend/e2e/doublons.spec.ts` le vérifie sur `/validation` en conditions réelles (le seed live ajoute la fiche `7792-SO-BIS` partageant le PDF de `7792-SO` puis lance le scan) et échoue si un contrôle de fusion apparaît ; (C.3) `docs/API.md` documente désormais les routes L.1, les routes `/auth/*`, la frontière de confiance, le verrouillage en `audit_log` et les 9 indicateurs.

**Portes** : suite locale 532 passés / 3 sautés ; suite PostgreSQL réelle `-m "postgres and not perf and not sauvegarde"` **185 passés / 1 sauté / 0 échec** (153 de base + 14 dédup + 18 comptes) ; `TABLES_METIER` contient **32** tables et `SESSION_UI` y figure ; `ruff check .` propre ; CI 8 jobs verte (4 gardes ROUGE→VERT en annotations) ; e2e live étendu (validation 3 tests, doublons 2, auth nominatif 23) ; RG13 aucune écriture d'archive, RG14 aucun appel réseau.
