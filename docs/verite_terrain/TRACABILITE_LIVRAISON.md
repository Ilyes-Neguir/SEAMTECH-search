...
## Lot J — facette dimension, URL partageable, journal exploité (22/09/2026, `arena/01a0ca57-seamtech-search`)

| Affirmation | Commande | Sortie brute | Commit | Statut |
|---|---|---|---|---|
| Facette DIMENSION : 7 cotes explicites avec unités métier, filtre par plage, intervalles depuis données réelles | `pytest -m postgres tests/test_recherche_dimension_tri.py` | 7 cotes (slu_m m, sle_m m, sf_m m, shw_m m, spa_m2 m², tetiere_cm cm, poids_kg kg), intervalles calculés, effectifs | 225516c | ⚠️ à prouver en CI (postgres) |
| Filtre dimension compte sans son propre filtre (règle des facettes) | `test_dimension_facettes_et_intervalles` | effectif slu_m avec filtre = sans filtre | 225516c | ⚠️ à prouver en CI |
| API étendue cote/min/max + alias cote_min/cote_max + tri 18 valeurs + page→offset | `pytest -m postgres tests/test_recherche_dimension_tri.py::test_api_dimension_params` + `test_tri_et_pagination` | codes filtrés {0701-GV-001, 0801-GEN-001}, page=2, tri code_asc | 225516c | ⚠️ à prouver en CI |
| Tri appliqué sur fusion AVANT pagination | `test_tri_et_pagination` | page1 max <= page2 min pour tri slu_m_asc | 225516c | ⚠️ à prouver en CI |
| URL partageable : q+filtres+tri+page dans URL, restaurés, copier lien, F5, nouvel onglet | `pnpm exec playwright test e2e/recherche-url.spec.ts` | 3 passed (question+filtres+tri+page restaurés, copier lien, F5, nouvel onglet, facette 7 cotes, pagination) | 225516c | ⚠️ à prouver en CI (e2e) |
| Journal exploité : top requêtes + sans résultat agrégés depuis table réelle, période paramétrable | `pytest -m postgres tests/test_journal_recherche.py::test_journal_rapport_exactitude_sur_jeu_injecte` | jeu injecté 6 lignes → total 6, sans 3, top exact, période 1j <= total | 225516c | ⚠️ à prouver en CI |
| Route /recherche/journal + CLI recherche-log | `python -m seamtech_search.journal_recherche --help` + `GET /recherche/journal` | help OK, route retourne total_recherches, top, sans | 225516c | ✅ établi (help mesuré §2.6 rapport) |
| Pas de régression : 487 passed/2 skipped hors postgres + ruff + audit 12/12 + tsc + build | `pytest -k "not postgres..."` + `ruff check` + `audit_projet.py --rapide` + `pnpm build` | 487 passed, All checks passed, 12/12, build 21 routes dont /api/recherche/journal | 225516c | ✅ établi (mesuré §2.3-2.5) |
| CI 8 jobs verte Lot J | `gh run view <id>` | attendu 8/8 success, e2e 3 passed, postgres passed=134, notices perf | 225516c | ❌ NON PROUVÉ au moment du rapport (push 35767854341 en cours) |
