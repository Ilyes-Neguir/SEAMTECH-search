...
export async function POST(req: NextRequest) {
  ...
  if (!payload?.source_path || !payload?.technical_pdf) {
    return NextResponse.json({ detail: "source_path and technical_pdf are required." }, { status: 400 })
  }
  const base = backendBase()
  if (!base) return NextResponse.json({ detail: "Import requires a configured backend." }, { status: 503 })
  try {
    const response = await fetch(`${base}/imports/confirm`, {
      method: "POST",
