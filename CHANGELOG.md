## Unreleased — Préparation release candidate + audit du stockage objet S3/MinIO (`arena/01a0d9a7-seamtech-search`)

**Périmètre : documentation + tests, aucun code applicatif modifié.** Aucun backend de stockage changé, aucun nom de bucket ni schéma de clé touché, aucun contrat d'API modifié, aucun test existant supprimé, aucun seuil de couverture abaissé.

**Audit** `docs/verite_terrain/AUDIT_STOCKAGE_S3_MINIO.md` : architecture, **12 opérations S3** réellement appelées (toutes dans `storage.py`, avec numéros de ligne), compatibilités AWS attendues, dépendances MinIO M-1→M-8 (infrastructure et CI uniquement — le code est portable), tests supposant MinIO, variables d'environnement, replis mesurés (`not_configured`/`not_applicable`/`partial`/`failed`, purge conditionnée à `all_verified`, quarantaine jamais purgée), statut du projet MinIO archivé (archivage du dépôt le 2026-04-25, images et binaires retirés, plus de canal de correctifs), risques **D-1 / R-1→R-14**, stratégie de test à 3 niveaux + suite de conformité en 11 points à exiger d'un endpoint candidat, grille de choix d'alternative (Garage, SeaweedFS, Ceph RGW, AWS S3, R2, fournisseur UE), plan de migration sans perte en 10 étapes (clés inchangées ⇒ aucune écriture en base, rollback = 5 variables), décisions **D-1→D-9** réservées au commanditaire. Conclusion : **VALIDÉ AVEC RÉSERVES**.

**Défaut confirmé par exécution (D-1, non corrigé — décision commanditaire)** : `api.py` l.477 et l.988 appellent `get_presigned_url(object_key, expires_in=900)` alors que la signature est `get_presigned_url(remote_key, expiration_seconds=3600)` ; le `TypeError` est avalé par `except Exception` ⇒ **la redirection 302 vers une URL présignée n'est jamais servie** (sortie mesurée : `WARNING Failed to generate presigned URL … unexpected keyword argument 'expires_in'`, `STATUT 200, Location: None`, boto3 jamais appelé). README et `docs/API.md` (« 302s to presigned URL, 900s expiry ») sont donc faux sur ce point.

**Défaut du dispositif de test (R-14)** : la CI désélectionne par sous-chaîne de nom (`pytest -k "not postgres and not s3"`, ci.yml l.91 et l.479) ⇒ `pytest -k "s3 and not postgres and not perf"` = **10 passed, 1 skipped** de tests existants qui ne s'exécutent **nulle part** en CI. Les tests ajoutés ont été nommés en conséquence (sans la chaîne `s3`), sinon 5 des 20 auraient été silencieusement ignorés.
...
**Tests** `tests/test_stockage_release_candidate.py` (**20**, aucun réseau, aucun vrai secret) : configuration du stockage objet complète/incomplète (R-2) et `storage_backend` cosmétique (R-3) ; absence de secret dans les journaux (envoi, 4 chemins d'erreur `ClientError`, URL présignée jamais journalisée) ; URL présignée (paramètres, signature publique = contrat, écart D-1, asymétrie de préfixe R-4, route artefact sans 500 ni fuite) ; `object_key` avec espaces/accents + NFC≠NFD (R-5) ; repli local et interdiction de purge après échec partiel ; démarrage sur base vide et survie des `object_key` au redémarrage ; migrations 001→017 séquentielles et idempotentes + `VERSION_SCHEMA_METIER`.

**Vérifications** : `ruff check .` → `All checks passed!` ; `git diff --check` vide ; `audit_projet.py --rapide` 12/12 ; `pytest -k "not postgres and not s3"` → **657 passed, 5 skipped** (637 avant le lot) ; couverture avec la commande CI → **69,7 % sans PostgreSQL, comme avant le lot (69,6 %)**, `storage.py` 97,5 % → **97,8 %**, `config.py` 86,0 % → **89,5 %** — seuils inchangés, la CI backend avec PostgreSQL reste le juge. Non exécutés et déclarés : build frontend (`node_modules`/`pnpm` absents, aucun fichier frontend modifié, job CI `frontend` vert), suite PostgreSQL, endpoint S3 réel, Docker. Rapport détaillé : `docs/verite_terrain/RAPPORT_20260925_RELEASE_CANDIDATE_STOCKAGE.md`. **Rappels : Lot G réel bloqué (archive non livrée), F-2 bloqué (fiches validées manquantes), métriques de production indisponibles, fixtures du dépôt non représentatives.**
