...
**Date** : 2026-09-25 · **Base** : `origin/main` = `f8befb5da800c10d793e25addf9519cc63f5fd6c`
(fusion de la PR #28) · **Nature** : audit **en lecture seule**.

> **Ce lot ne remplace pas MinIO.** Aucun backend de stockage, nom de bucket, schéma
> de clé d'objet, contrat d'API ni test existant n'a été modifié. Ce document
> constate, mesure, liste les risques et prépare une décision ; il ne la prend pas.
...
| `tests/test_integration_docker.py` | `docker compose up -d postgres redis minio`, endpoint `http://127.0.0.1:9000`, bucket `seamtech-documents`, mode `STRICT` en CI | rouge en CI (sauté hors Docker) |
| `tests/test_storage.py::test_live_minio_s3_integration` (marqueur `s3`) | endpoint réel via `SEAMTECH_TEST_S3_URL` | **sauté** (jamais exécuté en CI actuelle) |
| Job CI `sauvegarde` | `docker run … minio server /data`, bucket `seamtech-backups` créé par boto3, sonde `/minio/health/live` | **rouge** |
| `tests/test_sauvegarde_unites.py` / `…_restauration.py` | double `S3EnMemoire` (aucun MinIO requis) — seul le **job** CI utilise MinIO réel | verts |
...
|---|---|---|---|
| **D-1** | `api.py` appelle `get_presigned_url(object_key, expires_in=900)` alors que le paramètre s'appelle `expiration_seconds` → `TypeError` : **la redirection 302 vers une URL présignée n'est jamais servie**, c'est toujours le repli local (ou un téléchargement S3 puis un flux via l'API) | 🔴 | mesuré, §6 |
...
| R-14 | La CI écarte les tests par **mot-clé** (`pytest -k "not postgres and not s3"`, `.github/workflows/ci.yml` l.91, idem l.479 pour la couverture) : **tout test dont le nom contient `s3` est désélectionné**, y compris ceux qui n'ont besoin d'aucun endpoint. Mesuré le 2026-09-25 : `pytest -k "s3 and not postgres and not perf"` → **10 passed, 1 skipped** — 10 tests existants (chaos S3, configuration du client, `/open` avec `object_key`, rétention symétrique…) passent en local mais **ne sont jamais exécutés en CI** ; seul `test_live_minio_s3_integration` (marqueur `s3`) a vraiment besoin d'un endpoint | 🟠 | mesure ci-contre ; correctif = passer du filtre `-k "not s3"` au marqueur `-m "not s3"` — **non appliqué dans ce lot** (modifie le périmètre de la CI et la couverture mesurée : décision D-9) |
...
## 6. Écart D-1 — preuve mesurée

**Commande** (doublure boto3, aucun réseau — script jetable, non versionné) :
...
**Exécution de bout en bout** (route `/imports/{id}/artifacts/report_pdf`, vrai
`S3StorageClient`, boto3 doublé) :
...
- Ce n'est **pas** une perte de données ni une panne : la pièce est servie par le
  repli documenté (fichier local, ou téléchargement depuis S3 puis flux par l'API).
- C'est en revanche : (a) une **divergence documentation/réalité** — README et
  `docs/API.md` annoncent « 302s to presigned URL (900s expiry) » ; (b) un surcoût de
  bande passante et de latence (tout transite par l'API au lieu d'aller du navigateur
  au bucket) ; (c) l'affaiblissement de l'argument « zéro rétention locale », puisque
  le chemin qui s'exécute est celui qui dépend d'une copie locale ou d'un cache.
- Les tests existants ne l'attrapent pas : ils remplacent le client par un `MagicMock`,
  lequel accepte n'importe quel mot-clé.
- **Correctif candidat** (2 mots, non appliqué dans ce lot car il change un code de
  statut observable — décision D-1 §9) :
  ...
  à appliquer aux lignes 477 et 988, puis à couvrir par le test inverse (le test
  `test_ecart_d1_api_appelle_l_url_presignee_avec_un_mot_cle_inexistant` est écrit pour
  devenir rouge ce jour-là : il est fait pour être remplacé).
...
| **Conformité fournisseur** | `tests/test_storage.py::test_live_minio_s3_integration` (marqueur `s3`, `SEAMTECH_TEST_S3_URL`) | oui | qu'un endpoint réel honore envoi / relecture / téléchargement / présignature — **aujourd'hui jamais exécuté en CI** |
...
`tests/test_stockage_release_candidate.py` — **20 tests**, aucun réseau, aucun vrai
secret. Les noms évitent volontairement la chaîne `s3` : le filtre de la CI
(`-k "not postgres and not s3"`) désélectionne par **sous-chaîne du nom**, et cinq de
ces tests ne s'exécutaient pas tant qu'ils s'appelaient `…_s3_…` (voir R-14).
...
|---|---|---|---|
| **D-1** | Corriger l'appel d'URL présignée (`expires_in` → `expiration_seconds`) ? | (a) corriger et rétablir le 302 documenté ; (b) garder le repli et **corriger la documentation** | la documentation reste fausse et le trafic des pièces continue de transiter par l'API |
...
| **D-9** | Remplacer le filtre CI `-k "not s3"` par le marqueur `-m "not s3"` (R-14) | oui / non | 10 tests existants continuent de n'être exécutés nulle part en CI |
...
- Réserves à lever avant mise en production : **D-1** (redirection présignée jamais
  servie), **R-1** (MinIO sans amont), **R-2/R-3** (réglages trompeurs), **R-13**
  (compatibilité non prouvée hors MinIO), **R-14** (10 tests existants désélectionnés
  en CI par un filtre de mot-clé), et les décisions D-2 à D-9.
