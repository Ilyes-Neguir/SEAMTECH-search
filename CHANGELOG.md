...
**Vérifications** : `ruff check .` → `All checks passed!` ; `git diff --check` vide ; `audit_projet.py --rapide` **12/12** ; suite CI sans service → **715 passed, 3 skipped, 207 deselected** ; couverture (commande CI, sans PostgreSQL) **69,7 %**, inchangée par rapport à la veille, `storage.py` 97,5 % → **98,1 %**, `api.py` 87,4 % → **87,7 %** — **aucun seuil modifié**, la CI backend avec PostgreSQL reste le juge. Non exécutés et déclarés : build frontend (`node_modules`/`pnpm` absents, aucun fichier frontend modifié, job CI `frontend` vert), suite PostgreSQL, endpoint S3 réel, Docker. Rapport détaillé : `docs/verite_terrain/RAPPORT_20260925_RELEASE_CANDIDATE_STOCKAGE.md` ; addendum au rapport du 24/09 sur les deux mesures dont la base a changé. **Rappels : Lot G réel bloqué (archive non livrée), F-2 bloqué (fiches validées manquantes), métriques de production indisponibles, fixtures du dépôt non représentatives.**
